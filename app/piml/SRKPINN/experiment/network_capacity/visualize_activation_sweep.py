"""Visualize the fixed-time SRKPINN pendulum activation sweep results."""
...
SUMMARY_SUFFIX = os.environ.get("SRKPINN_ACT_SUMMARY_SUFFIX", "").strip().lower().replace(".", "p").replace("-", "m")
SUMMARY_STEM = f"activation_summary_fixed_time_T20_{SUMMARY_SUFFIX}" if SUMMARY_SUFFIX else "activation_summary_fixed_time_T20"
FIGURE_STEM = f"activation_fixed_time_T20_{SUMMARY_SUFFIX}" if SUMMARY_SUFFIX else "activation_fixed_time_T20"
...
def row_sort_key(row: dict) -> tuple[str]:
    return (str(row["activation"]),)
...
def build_row_label(row: dict) -> str:
    return f"{row['activation']}\nw={int(row['hidden_width'])}\nd={int(row['hidden_depth'])}"
...
def create_metric_overview(rows: list[dict], eval_total_time: float) -> None:
    ...
    fig.suptitle("SRKPINN Activation Sweep Comparison (Fixed Physical Horizon)", fontsize=15, fontweight="bold")
    fig.tight_layout()
...
def create_panel_montage(rows: list[dict]) -> None:
    ...
    fig.suptitle("SRKPINN Final Diagnostic Panels by Activation Setting (T=20)", fontsize=18, fontweight="bold")
    fig.tight_layout()
