"""Network-capacity follow-up: width sweep with fixed best optimization settings."""
...
def summary_stem(summary_suffix: str) -> str:
    ...
    return f"width_summary_fixed_time_T20{suffix}"
...
def build_run_name(
    ...
    base_name = (
        f"width_T20_lr_{sanitize_float(learning_rate)}_ep_{num_epochs}_w_{width}_d_{hidden_depth}_{sanitize_token(activation)}"
    )
    ...
    *,
    ...
    for row in rows:
        if row["status"] == "completed":
            ...
            lines.append(
                "| "
                f"`{row['run_name']}` | `completed` | `{row['width']}` | `{row['layers']}` | "
            ...
            lines.append(
                f"| `{row['run_name']}` | `failed` | `{row['width']}` | `{row['layers']}` | "
                "`-` | `-` | `-` | `-` | `-` | `-` | `-` | `-` | `-` |"
            )
            lines.append("")
...
def main() -> None:
    seed = env_int("SRKPINN_WIDTH_SEED", 2026)
    learning_rate = env_float("SRKPINN_WIDTH_LR", LEARNING_RATE)
    num_epochs = env_int("SRKPINN_WIDTH_EPOCHS", NUM_EPOCHS)
    widths = env_int_list("SRKPINN_WIDTH_VALUES", DEFAULT_WIDTHS)
    hidden_depth = env_int("SRKPINN_WIDTH_DEPTH", HIDDEN_DEPTH)
    activation = env_str("SRKPINN_WIDTH_ACTIVATION", ACTIVATION)
    log_freq = env_int("SRKPINN_WIDTH_LOG_FREQ", 100)
    history_freq = env_int("SRKPINN_WIDTH_HISTORY_FREQ", log_freq)
    checkpoint_freq = env_int("SRKPINN_WIDTH_CHECKPOINT_FREQ", 1000)
    run_tag = env_str("SRKPINN_WIDTH_RUN_TAG", "")
    summary_suffix = env_str("SRKPINN_WIDTH_SUMMARY_SUFFIX", "")

    rows = []

    for width in widths:
        layers = build_layers(state_dim=2, stages=STAGES, hidden_width=width, hidden_depth=hidden_depth)
        run_name = build_run_name(run_tag, learning_rate, num_epochs, width, hidden_depth, activation)
        ...
        try:
            config, summary = run_pendulum_experiment(
                ...
                summary_title="SRKPINN Width Sweep Summary (Fixed Physical Horizon)",
            )
            rows.append(
                {
                    "run_name": run_name,
