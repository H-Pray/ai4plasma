"""Visualize the fixed-time SRKPINN pendulum width sweep results."""
...
SUMMARY_SUFFIX = os.environ.get("SRKPINN_WIDTH_SUMMARY_SUFFIX", "").strip().lower().replace(".", "p").replace("-", "m")
SUMMARY_STEM = f"width_summary_fixed_time_T20_{SUMMARY_SUFFIX}" if SUMMARY_SUFFIX else "width_summary_fixed_time_T20"
FIGURE_STEM = f"width_fixed_time_T20_{SUMMARY_SUFFIX}" if SUMMARY_SUFFIX else "width_fixed_time_T20"
...
def create_metric_overview(rows: list[dict], eval_total_time: float) -> None:
    ...
    fig.suptitle("SRKPINN Width Sweep Comparison (Fixed Physical Horizon)", fontsize=15, fontweight="bold")
    fig.tight_layout()
...
def create_panel_montage(rows: list[dict]) -> None:
    ...
    fig.suptitle("SRKPINN Final Diagnostic Panels by Width Setting (T=20)", fontsize=18, fontweight="bold")
    fig.tight_layout()
