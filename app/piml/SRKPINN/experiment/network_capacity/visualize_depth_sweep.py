"""Visualize the fixed-time SRKPINN pendulum depth sweep results."""
...
SUMMARY_SUFFIX = os.environ.get("SRKPINN_DEPTH_SUMMARY_SUFFIX", "").strip().lower().replace(".", "p").replace("-", "m")
SUMMARY_STEM = f"depth_summary_fixed_time_T20_{SUMMARY_SUFFIX}" if SUMMARY_SUFFIX else "depth_summary_fixed_time_T20"
FIGURE_STEM = f"depth_fixed_time_T20_{SUMMARY_SUFFIX}" if SUMMARY_SUFFIX else "depth_fixed_time_T20"
...
def build_row_label(row: dict) -> str:
    return f"d={int(row['hidden_depth'])}\nw={int(row['hidden_width'])}\n{row['activation']}"
...
def create_metric_overview(rows: list[dict], eval_total_time: float) -> None:
    ...
    fig.suptitle("SRKPINN Depth Sweep Comparison (Fixed Physical Horizon)", fontsize=15, fontweight="bold")
    fig.tight_layout()
...
def create_panel_montage(rows: list[dict]) -> None:
    ...
    fig.suptitle("SRKPINN Final Diagnostic Panels by Depth Setting (T=20)", fontsize=18, fontweight="bold")
    fig.tight_layout()
