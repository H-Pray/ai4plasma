"""Visualize the fixed-time SRKPINN pendulum optimization sweep results."""
...
SUMMARY_SUFFIX = os.environ.get("SRKPINN_OPT_SUMMARY_SUFFIX", "").strip().lower().replace(".", "p").replace("-", "m")
SUMMARY_STEM = f"aggregate_summary_fixed_time_T20_{SUMMARY_SUFFIX}" if SUMMARY_SUFFIX else "aggregate_summary_fixed_time_T20"
FIGURE_STEM = f"fixed_time_T20_{SUMMARY_SUFFIX}" if SUMMARY_SUFFIX else "fixed_time_T20"
...
def build_row_label(row: dict, show_epochs: bool, show_scheduler: bool) -> str:
    parts = [f"lr={float(row['learning_rate']):.1e}"]
    if show_epochs:
        parts.append(f"ep={int(row['num_epochs'])}")
    if show_scheduler:
        parts.append(str(row["scheduler_name"]))
    return "\n".join(parts)
...
def create_metric_overview(rows: list[dict], eval_total_time: float) -> None:
    ...
    epochs = {int(row["num_epochs"]) for row in rows}
    schedulers = {str(row["scheduler_name"]) for row in rows}
    show_epochs = len(epochs) > 1
    show_scheduler = len(schedulers) > 1
    labels = [build_row_label(row, show_epochs, show_scheduler) for row in rows]
    ...
    for row, rollout_value, energy_value in zip(rows, rollout, energy):
        ...
        label = build_row_label(row, show_epochs, show_scheduler).replace("\n", ", ")
    ...
    fig.suptitle("SRKPINN Optimization Sweep Comparison (Fixed Physical Horizon)", fontsize=15, fontweight="bold")
    fig.tight_layout()
...
def create_panel_montage(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        images.append(image)
        labels.append(build_row_label(row, show_epochs, show_scheduler).replace("\n", ", "))
    ...
    fig.suptitle("SRKPINN Final Diagnostic Panels by Optimization Setting (T=20)", fontsize=18, fontweight="bold")
    fig.tight_layout()
